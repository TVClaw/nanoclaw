...
async function main(): Promise<void> {
  if (!fs.existsSync(path.join(AUTH_DIR, 'creds.json'))) {
    console.error('No WhatsApp session. Scan the QR in the step above first.');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      'Create a group named "' +
        groupName +
        '" manually in WhatsApp, then: WHATSAPP_SKIP_BOOTSTRAP=1 npm run link:whatsapp and paste its JID.',
    );
    sock.end(undefined);
    process.exit(1);
  }
  ...
  console.log(
    '\nOnly this group is registered. Message-yourself stays private until you register it.\nYou can talk to NanoClaw directly in this group without @ mention.\n',
  );
